...
/// physically move a status item along the real menu bar (milestones 0.5 path (a),
/// per-item auto-placement).
///
...
/// design, so it runs on a user-initiated action, not silently. Untested here like the
/// other AppKit/CGEvent glue; it rides with the on-device validation in milestone 0.9.
