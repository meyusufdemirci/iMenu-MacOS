...
| 0.5 | **Remove/hide a chosen item from the real menu bar** — the central bet. Decide and prove one path: **(a) active** — own a divider status item + move a specific third-party item past it, then collapse it off-screen; or **(b) passive** — only mirror items macOS *already* clipped (no moving). *(a) is what makes a manual Visible↔Hidden toggle actually control the system bar.* | Core mechanic | 🟡 **Path (a) chosen & coded.** `Features/SystemBar/*` — iMenu owns a divider `NSStatusItem` (`DividerMenuBarCollapser`); collapsing widens its slot so the items the user parked to its **left** leave the visible bar, driven live by `LayoutStore` (Hidden non-empty → collapse) behind the injected `SystemMenuBarCollapsing` seam. Uses the reliable **divider trick** (Hidden Bar / Dozer), **not** raw synthesized ⌘-drag — the user ⌘-drags items past the divider once. The expand-length rule is pure + unit-tested (`MenuBarCollapseGeometry`); the store wiring is spy-tested. Built to the repo's product conventions (TDD/`L10n`/`AppLogger`), not throwaway. **✅ Primitive PROVEN on device (macOS 26, 2026-07-10):** widening iMenu's divider *does* push **other apps'** icons off the visible menu bar — the central bet holds. Verified by hand via a debug toggle in the `MenuBarExtra` menu + an eagerly-installed, click-to-toggle divider. Residual for a shipping feature: **(1) per-item auto-placement** — today the user must ⌘-drag items to the divider's left by hand; making the Layout "move to Hidden" hide a *chosen* item automatically still needs synthesized ⌘-drag (the remaining fragile unknown); **(2) UX** — the single divider doubles as the click target, so it slides off-screen when collapsed; productionize with the two-item design (fixed toggle + separate spacer). On-device notch/external-display/Spaces coverage still rides with 0.9. |
...
2. ~~**Making a chosen item leave the real menu bar** (0.5)~~ — **✅ proven on device (macOS 26):** widening iMenu's divider pushes other apps' items off the visible bar. *Remaining before it's a real feature: per-item auto-placement (synthesized ⌘-drag so the Layout "move to Hidden" hides a chosen item without a manual drag) and the two-item toggle UX.*
